...
def TrackCorners(prev_corners, prev_image, current_image):
    klt = LucasKanadeTracker(marker_points=prev_corners)
    flows = klt.track(prev_image, current_image, parallel=False)
    ...
    for i in range(len(flows)):
        ...
        new_corners[i, 0] = x + u
        new_corners[i, 1] = y + v
    # flows = np.array([flow[1] for flow in flows])

    # new_corners = prev_corners + flows
    return new_corners
