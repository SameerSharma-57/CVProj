...
'''
def Render(input_video_path, output_path, obj_file_path):
    extrinsic_matrices_path = f"{output_path}/arrays/extrinsics.npz"
    intrinsics_path = f"{output_path}/arrays/calib_data.npz"
    os.makedirs(f"{output_path}/temp_render_images", exist_ok=True)
    ...
    for i in tqdm(range(n_frames), desc="Processing frames"):
        ...
        cv2.imwrite(f"{output_path}/temp_render_images/output_{frame_number}.png", final_frame)
    ...
    cap.release()
    images_to_video(f"{output_path}/temp_render_images", f"{output_path}/videos/3_render.mp4", fps=30)

    shutil.rmtree(f"{output_path}/temp_render_images")
