...
data = np.load(f"{output_folder}/calib_data.npz")
camMatrix = data["camMatrix"]
distCof = data["distCoef"]
'''
def CalibrateCamera(checkerboard_video_path, output_folder, square_size = 16, n_images=40):
        # Create temporary folder to store calibration images
        calib_images_folder = f"{output_folder}/temp_calib_images"
        intrinsics_save_folder = f"{output_folder}/arrays"
        os.makedirs(output_folder, exist_ok=True)
        os.makedirs(calib_images_folder, exist_ok=True)
        os.makedirs(intrinsics_save_folder, exist_ok=True)

        # Debugging variables
        remove_temp = True # set to False to keep the temp folder for debugging
        log = False # set to False to disable logging
        ...
        save_path = f"{intrinsics_save_folder}/calib_data.npz"
